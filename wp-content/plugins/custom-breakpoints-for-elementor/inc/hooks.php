...
class JLTMA_Master_Custom_Breakpoint_Hooks {
    ...
    public function jltma_mcb_content() {
        ...
        if ( ! is_plugin_active( 'elementor/elementor.php' ) ) {
            echo sprintf("<h2>Please Active Elementor Plugin</h2>", JLTMA_MCB_TD);
            return;
        }
            ...
            <form method="POST" class="jlmta-cbp-input-form" id="jlmta-cbp-form">
                ...
                <div id="custom_breakpoints_page">
                    ...
                    <div id="master_cbp_table" class="master_cbp_table">
                        ...
                        <ul :class="['breakpoint-row', breakpoint.isRecent ? 'breakpoint-is-recent' : '' ]" v-for="(breakpoint, index) in all_breakpoints" :key="index" :data-key="breakpoint.key">
                            ...
                            <li class="bp-min-width">
                                <input type="number" :value="breakpoint.min" disabled v-if="in_array( breakpoint.key, default_devices )">
                                <input class="jlma-mcb-model jlma-mcb-model--min" type="number" :value="breakpoint.min" v-else @change="valueChanged($event, breakpoint, 'min')" @focus="inputFocused($event, breakpoint)">
                            </li>
                            ...
                            <li class="bp-max-width" v-else>
                                <input class="jlma-mcb-model jlma-mcb-model--max" type="number" :value="breakpoint.max" @change="valueChanged($event, breakpoint, 'max')" @focus="inputFocused($event, breakpoint)">
                            </li>
                    ...
                    <div class="submit">
                        <div :class="['button button-primary jltma-cbp-add', disable_add_breakpoint ? 'jltma-disabled' : '']" @click="add_device"><?php echo esc_html__('Add Breakpoint', JLTMA_MCB_TD);?></div>
    ...
    public function jltma_mcb_save_settings(){
        ...
        foreach ( $form_fields as $key => $value ) {
            ...
            if ( $value['input1'] < 1 || $value['input2'] < 1 ) continue;
            ...
            $custom_breakpoints["breakpoint{$key}"] = [
                ...
                'input1'   => (int) $value['input1'],
                'input2'   => (int) $value['input2']
            ];
        ...
        $config = array(
            ...
            'strings'      => array(
                ...
                'notice_can_install_required'     => _n_noop(
                    'Custom Breakpoints requires the following plugin: %1$s.',
                    'Custom Breakpoints requires the following plugins: %1$s.',
                    'master-custom-breakpoint'
                ),
                'notice_can_install_recommended'  => _n_noop(
                    'Custom Breakpoints recommends the following plugin: %1$s.',
                    'Custom Breakpoints recommends the following plugins: %1$s.',
                    'master-custom-breakpoint'
                ),
                'notice_ask_to_update'            => _n_noop(
                    'The following plugin needs to be updated to its latest version to ensure maximum compatibility with Custom Breakpoints: %1$s.',
                    'The following plugins need to be updated to their latest version to ensure maximum compatibility with Custom Breakpoints: %1$s.',
                    'master-custom-breakpoint'
                ),
                ...
                'plugin_needs_higher_version'     => __( 'Plugin not activated. A higher version of %s is needed for Custom Breakpoints. Please update the plugin.', 'master-custom-breakpoint' ),
    ...
    public function hooks_scripts( $hook ) {
        ...
        wp_register_script( 'jltma-vue-js', JLTMA_MCB_PLUGIN_URL . 'assets/js/vue.min.js', [], JLTMA_MCB_VERSION, true );
        ...
        $data = [
            ...
            'devices' => Responsive::get_devices(),
